...
<div class="matchType space-y-4">
  {data.schedule.map((launch)=>
    (<div class="sm:container h-64 w-80 hover:min-w-0 rounded-lg text-center uppercase text-2xl text-slate-400 font-mono font-bold text-xl border-zinc-900 bg-slate-600 space-y-4">{launch.matchStatus}        
    <div class="sm:container h-20 w-64 ml mt-5 justify-center text-center uppercase text-xl text-gray-900 rounded lg font-semibold border-3 bg-slate-100 font-serif" >{launch.homeTeamName}<div class=" text-base italic text-red-600 font-semibold font-mono">{launch.matchType}</div> {launch.awayTeamName}
        </div>
        <div class="sm:container h-10 w-64 ml mt-15 justify-center text-center capitalize text-xl font-mono rounded lg text-green-600 border-3 bg-slate-100">{launch.venue}</div>
...
<div class="sm:container w-64 hover:min-w-0 rounded-lg border-zinc-900 bg-slate-600">
